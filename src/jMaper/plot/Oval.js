jMaper.Geom.Oval = jMaper.Geom.Extend({

	namespace : 'jMaper.Layer.Geom.Oval',
				...
				default:
				{
					msg = this.matte || !this._arise(this.arise);
					{
						if(this.handle){
							...
							this.handle = this.target.sketch.createOval(msg ? {matte : msg} : {
								title : this.title,
								mouse : this.mouse,
								alpha : this.alpha,
								color : this.color,
			...
			(function(){
				if(this.target)
					if(!this.handle){
						...
						// 回调面积
						try{
							fun(this.$.Match.isNumber((this.extra || 0).x) && this.$.Match.isNumber((this.extra || 0).y) ? Math.round(this.extra.x * this.extra.y * Math.PI * 100) / 100 : 0.0);
						}catch(e){
							throw e;
						}finally{
							fun = null;
