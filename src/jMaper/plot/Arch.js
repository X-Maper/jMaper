jMaper.Geom.Arch = jMaper.Geom.Extend({

	namespace : 'jMaper.Layer.Geom.Arch',
	...
	structure : function(jsn){
		if((this.radio = {}) && jsn){
			...
			if(this.$.Match.isObject(jsn.angle))
				this.angle = jsn.angle;
				...
				default:
				{
					msg = this.matte || !this._arise(this.arise);
					{
						if(this.handle){
							...
							this.handle = this.target.sketch.createArch(msg ? {matte : msg} : {
								title : this.title,
								mouse : this.mouse,
								alpha : this.alpha,
								color : this.color,
